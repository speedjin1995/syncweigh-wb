...
$selectedId = isset($_POST['selectedId']) ? intval($_POST['selectedId']) : null;

if($selectedId) {
    $sql = "SELECT * FROM Payment_Voucher WHERE (outstanding_amount > 0 OR id = ?) AND deleted = '0' ORDER BY voucher_no ASC";
    if ($update_stmt = $db->prepare($sql)) {
        $update_stmt->bind_param('i', $selectedId);
