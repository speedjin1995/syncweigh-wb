...
if (isset($_POST['addressLine1'], $_POST['companyName'])) {
    ...
    if (empty($_POST["companyCode"])) {
        ...
        $transporterCode = trim($_POST["companyCode"]);
    }
    ...
    if(! empty($transporterId))
    {
        ...
        $action = "2";
        if ($update_stmt = $db->prepare("UPDATE Company SET company_code=?, name=?, company_reg_no=?, address_line_1=?, address_line_2=?, address_line_3=?, phone_no=?, fax_no=?, created_by=?, modified_by=? WHERE id=?")) 
        {
            $update_stmt->bind_param('sssssssssss', $transporterCode, $companyName, $companyRegNo, $addressLine1, $addressLine2, $addressLine3, $phoneNo, $faxNo, $username, $username, $transporterId);
    ...
    {
        $action = "1";
        if ($insert_stmt = $db->prepare("INSERT INTO Company (company_code, name, company_reg_no, address_line_1, address_line_2, address_line_3, phone_no, fax_no, created_by, modified_by) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)")) {
            $insert_stmt->bind_param('ssssssssss', $transporterCode, $companyName, $companyRegNo, $addressLine1, $addressLine2, $addressLine3, $phoneNo, $faxNo, $username, $username);
