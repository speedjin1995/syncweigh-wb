...
if(isset($_POST['userID'])){
	...
	if ($type == 'MULTI'){
		...
		if ($stmt2 = $db->prepare("UPDATE Driver SET status=? WHERE id IN ($ids)")) {
			$stmt2->bind_param('s', $del);
			
			if($stmt2->execute()){
				// foreach($_POST['userID'] as $id){
				// 	if ($insert_stmt = $db->prepare("INSERT INTO Driver_Log (driver_id, action_id, action_by) VALUES (?, ?, ?)")) {
		...
		if ($stmt2 = $db->prepare("UPDATE Driver SET status=? WHERE id=?")) {
