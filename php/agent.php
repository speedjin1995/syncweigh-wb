...
if (isset($_POST['destinationCode'])) {
    ...
    if(! empty($destinationId))
    {
        ...
        $action = "2";
        if ($update_stmt = $db->prepare("UPDATE Agents SET destination_code=?, name=?, description=? , created_by=?, modified_by=? WHERE id=?")) 
        {
            ...
            if (! $update_stmt->execute()) {
                ...
                if ($insert_stmt = $db->prepare("INSERT INTO Agents_Log (agent_id, agent_code, name, description, action_id, action_by) VALUES (?, ?, ?, ?, ?, ?)")) {
