...
<head>
    <title>Camera Setup | Synctronix - Weighing System</title>
                                        ...
                                        <div class="modal-dialog modal-dialog-scrollable modal-lg">
                                            <div class="modal-content">
                                                ...
                                                <div class="modal-body">
                                                    <form role="form" id="cameraForm" class="needs-validation" novalidate autocomplete="off">
                                                                    ...
                                                                    <div class="card-body">
                                                                        <div class="row">
                                                                            <div class="col-xxl-12 col-lg-12 mb-3">
                                                                                <div class="row">
                                                                                    <label for="cameraName" class="col-sm-4 col-form-label"><?=$languageArray['name_code'][$language]?></label>
                                                                                    <div class="col-sm-8">
                                                                                        <input type="text" class="form-control" id="cameraName" name="cameraName" placeholder="Camera Name" required>
                                                                            ...
                                                                            <div class="col-xxl-12 col-lg-12 mb-3">
                                                                                <div class="row">
                                                                                    <label for="serialNo" class="col-sm-4 col-form-label"><?=$languageArray['serial_no_code'][$language]?></label>
                                                                                    <div class="col-sm-8">
                                                                                        <input type="text" class="form-control" id="serialNo" name="serialNo" placeholder="Serial Number" required>
                                                                                        <div class="invalid-feedback">
                                                                                            Please fill in the field.
                                                                                        </div>
                                                                                    </div>
                                                                            ...
                                                                            <div class="col-xxl-12 col-lg-12 mb-3" id="activeDiv" style="display:none;">
                                                                                <div class="row">
                                                                                    <label for="active" class="col-sm-4 col-form-label"><?=$languageArray['active_code'][$language]?></label>
                                                                                    <div class="col-sm-8">
                                                        ...
                                                        <div class="col-lg-12">
                                                            <div class="hstack gap-2 justify-content-end">
                                                                ...
                                                                <button type="button" class="btn btn-success" id="submitCamera"><?=$languageArray['submit_code'][$language]?></button>
                                                            </div>
                                                        </div><!--end col-->                                                               
                                                    </form>
                                                </div>
                                            ...
                                            <div class="modal-content">
                                                <form role="form" id="uploadForm">
                                                    ...
                                                    <div class="modal-footer justify-content-between bg-gray-dark color-palette">
                                                        ...
                                                        <button type="button" class="btn btn-success" id="uploadCamera"><?=$languageArray['submit_code'][$language]?></button>
                                                    </div>
                                                </form>
                                            </div>
                                        </div>
                                        ...
                                        <div class="row">
                                            <div class="col-lg-12">
                                                <div class="card">
                                                    <div class="card-header">
                                                        <div class="d-flex justify-content-between">
                                                            ...
                                                            <div class="flex-shrink-0">
                                                                ...
                                                                <button type="button" id="addCamera" class="btn btn-success waves-effect waves-light" data-bs-toggle="modal" data-bs-target="#addModal">
                                                    ...
                                                    <div class="card-body">
                                                        <table id="cameraTable" class="table table-bordered nowrap table-striped align-middle" style="width:100%">
                                                            <thead>
                                                                <tr>
                                                                    <th><input type="checkbox" id="selectAllCheckbox" class="selectAllCheckbox"></th>
                                                                    <th><?=$languageArray['name_code'][$language]?></th>
                                                                    <th><?=$languageArray['serial_no_code'][$language]?></th>
                                                                    <th><?=$languageArray['active_code'][$language]?></th>
...
$(function () {
    $('#selectAllCheckbox').on('change', function() {
        var checkboxes = $('#cameraTable tbody input[type="checkbox"]');
        checkboxes.prop('checked', $(this).prop('checked')).trigger('change');
    });

    table = $("#cameraTable").DataTable({
        "responsive": true,
        "autoWidth": false,
        'processing': true,
        'serverSide': true,
    ...
    $('#submitCamera').on('click', function(){
        if($('#cameraForm').valid()){
            $('#spinnerLoading').show();
            $.post('php/camera.php', $('#cameraForm').serialize(), function(data){
    ...
    $('#addCamera').on('click', function(){
        ...
        $('#addModal').find('#cameraName').val("");
        $('#addModal').find('#serialNo').val("");
        $('#addModal').find('#active').val("Y");
        $('#activeDiv').hide();
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                if (obj.status === 'success') {
                    ...
                    $("#successBtn").attr('data-toast-text', obj.message);
                    $("#successBtn").click();
                    $('#cameraTable').DataTable().ajax.reload(null, false);
                } 
                else if (obj.status === 'failed') {
                    $('#spinnerLoading').hide();
                    $("#failBtn").attr('data-toast-text', obj.message );
    ...
    $('#multiDeactivate').on('click', function () {
        ...
        $("#cameraTable tbody input[type='checkbox']").each(function () {
            if (this.checked) {
        ...
        if (selectedIds.length > 0) {
            if (confirm('Are you sure you want to cancel these items?')) {
                $.post('php/deleteCamera.php', {userID: selectedIds, type: 'MULTI'}, function(data){
                    var obj = JSON.parse(data);
...
function edit(id){
    $('#spinnerLoading').show();
    $.post('php/getCamera.php', {userID: id}, function(data)
    {
        var obj = JSON.parse(data);
        if(obj.status === 'success'){
            $('#addModal').find('#id').val(obj.message.id);
            $('#addModal').find('#cameraName').val(obj.message.name);
            $('#addModal').find('#serialNo').val(obj.message.serial_number);
            $('#addModal').find('#active').val(obj.message.active);
            $('#activeDiv').show();
...
function deactivate(id){
    ...
    if (confirm('Are you sure you want to cancel this item?')) {
        $.post('php/deleteCamera.php', {userID: id}, function(data){
            var obj = JSON.parse(data);
